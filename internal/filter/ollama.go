...
근거: 연령 집단 전체를 부정적으로 일반화하는 표현입니다.
판정: quarantine

메시지: "그 동네 사람들은 좀 그렇더라"
근거: 특정 지역 집단을 막연히 부정적으로 일반화하는 표현입니다.
판정: quarantine

메시지: "오늘 버스에서 할아버지가 자리 양보를 안 해줬어"
근거: 개인 경험 서술이며 특정 집단 전체를 타깃하지 않습니다.
판정: allow

메시지: "틀딱들은 왜 저래"
근거: 노인 비하 신조어를 사용한 명확한 혐오 표현입니다.
판정: block

이제 아래 메시지를 판단하세요:
메시지: "%s"

다음 형식으로 정확히 두 줄만 답하세요. 근거는 반드시 존댓말(~입니다/~습니다)로 작성하세요:
근거: <한 문장으로 판단 근거>
